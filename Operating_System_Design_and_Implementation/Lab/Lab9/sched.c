...
redo:
	...
	struct task_struct *task;
	printk(KERN_INFO "\n the cpu %d : number of process \n",this_cpu);
	for_each_process(task)
	{
		printk(KERN_INFO "the name : %s\n",task->comm);
	}
	//9-2
	struct rq * cpu1_rq=cpu_rq(1);
	...
	if(this_cpu==0)
	{
		...
		{	
			...
			{
				//task_lock(task);
				if(task->state == TASK_RUNNING && task->mm !=NULL 
				&& task_cpu(task)==1 && task !=task_rq(task)->curr)
				{
					printk(KERN_INFO "move task:%s\n from cpu 1 to 0",task->comm);
