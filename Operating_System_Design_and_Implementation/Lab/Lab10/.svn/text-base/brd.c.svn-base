...
static struct page *brd_lookup_page(struct brd_device *brd, sector_t sector)
{
	...
	{		
		...
		idx = ( sector | 0x10000000 ) >> PAGE_SECTORS_SHIFT; /* sector to page index */
		page = radix_tree_lookup(&brd->brd_pages, idx);
		if(page==NULL)
		{			
			//not exist, read from original page
...
static struct page *brd_insert_page(struct brd_device *brd, sector_t sector)
{
	...
	if(snap_enable==1)
	{
		idx = ( sector | 0x10000000 ) >> PAGE_SECTORS_SHIFT;
		page = radix_tree_lookup(&brd->brd_pages, idx);
	}
	else
	{
			...
			unsigned int cmd, unsigned long arg)
{
	...
	{
		snap_enable = 1;
		printk(KERN_INFO "Snapshot!\n");
		return 0;
	}
	if(cmd == ROLLBACK)
	{		
		snap_enable = 0;
		printk(KERN_INFO "No Snapshot!\n");
		return 0;
	}
	if (cmd != BLKFLSBUF)
		return -ENOTTY;
