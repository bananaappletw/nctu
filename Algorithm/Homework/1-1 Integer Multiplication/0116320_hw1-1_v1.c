...
long long ten[9]={1,10,100,1000,10000,100000,1000000,10000000,max};
int main()
{
    ...
    for(k=0;k<N;k++)
    {
        ...
        */
        ...
        while(cbit+n*8<i)
            n++;
        cbegin=csize-1-n;
        cbeginbit=cbit+n*8-i+1;
        while(cbit+n*8<j)
            n++;
        cend=csize-1-n;
        cendbit=cbit+n*8-j+1;
        ...
        if(clength>1)
        {
            ...
            printf("%0*lld",8-cendbit+1,C[cend]/ten[cendbit-1]);
        }
        else
        {
